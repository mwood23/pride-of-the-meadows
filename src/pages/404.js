...
Runner.prototype = {
  ...
  loadSounds: function() {
    if (!IS_IOS) {
      ...
      var audioArray = Array.from(resourceTemplate);

      for (var sound in audioArray) {
        var soundSrc = audioArray[sound].src;
    ...
    if (e.target != this.detailsButton) {
      if (
        ...
        if (!this.tRex.jumping && !this.tRex.ducking) {
          console.log("jump", this.soundFx[0]);
          this.playSound(this.soundFx[0]);
  ...
  render() {
    return (
      ...
      <ContentContainer>
          ...
          <p>Tap the space bar or tap on the screen to play a game while we get to making it for you.</p>
        </ContentContainer>
      </div>
    );
  }
